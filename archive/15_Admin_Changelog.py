...
st.info("This page displays a log of all data changes made within the application, such as edits, additions, and deletions.")
...
if changelog_df is not None:
    ...
    with col2:
        ...
        min_date = changelog_df['Timestamp'].min().date()
        start_date = st.date_input("Start Date", value=min_date)
    
    with col3:
        # Filter by Date Range (End)
        max_date = changelog_df['Timestamp'].max().date()
        end_date = st.date_input("End Date", value=max_date)

    # Apply filters
    filtered_log = changelog_df.copy()
    ...
    filtered_log = filtered_log[
        ...
        (filtered_log['Timestamp'] < end_date_dt + pd.Timedelta(days=1)) # Add 1 day to include the end date fully
    ]
