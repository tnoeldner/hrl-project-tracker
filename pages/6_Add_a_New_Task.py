...
if df_original is not None:
    with st.form("new_task_form", clear_on_submit=True):
        ...
        semester = st.selectbox("Semester", options=sorted(df_original['SEMESTER'].unique().tolist()))
        fiscal_year = st.selectbox("Fiscal Year", options=sorted(df_original['Fiscal Year'].unique().tolist()))
        audience = st.selectbox("Audience", options=sorted(df_original['AUDIENCE'].unique().tolist()))
